...
case class RoleMembersUnionMember[+MemberType](
    value: MemberType,
    indicesInParents: Iterable[(WithMembers[_], Int)]
)
...
/**
  ...
  */
class RoleMembersUnion[+ComponentType <: Component](
    val linkedMembers: Seq[RoleMembersUnionMember[ComponentType]]
  ...
  private[tcof] override def mapChildToParent(
    ...
    for ((member, idx) <- members) {
      val vrs =
        for ((parent, indexInParent) <- member.indicesInParents)
          yield _solverModel.member(indexInParent, parent.allMembersVar).reify()

      _solverModel.ifOnlyIf(
        _solverModel.member(idx, membersContainer.allMembersVar),
